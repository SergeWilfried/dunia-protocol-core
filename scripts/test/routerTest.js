...
module.exports = async function(callback) {
  ...
  await cowrie.mint(accounts[0], feiTotal);
  await cowrie.approve(router.address, MAX_UINT256, {from: accounts[0]});
  ...
  let currentFei = await cowrie.balanceOf(ethPair.address);

  await cowrie.burnFrom(ethPair.address, currentFei, {from: accounts[0]});
  await cowrie.mint(ethPair.address, targetFei, {from: accounts[0]});
  await ethPair.sync();

  console.log('Selling At');
  reserves = await ui.getReserves();
  console.log(`Cowrie reserves=${stringify(reserves[0])}, Eth reserves=${stringify(reserves[1])}`);
  console.log(`User Cowrie balance=${stringify(await cowrie.balanceOf(accounts[0]))}`);
  ...
  reserves = await ui.getReserves();
  console.log(`Cowrie reserves=${stringify(reserves[0])}, Dunia reserves=${stringify(reserves[1])}`);
  console.log(`User Cowrie balance=${stringify(await cowrie.balanceOf(accounts[0]))}`);
  ...
  console.log(`Post-User Cowrie balance=${stringify(await cowrie.balanceOf(accounts[0]))}`);
